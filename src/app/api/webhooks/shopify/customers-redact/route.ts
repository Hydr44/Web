...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[shopify webhook customers/redact] incoming", {
      shopDomain,
    ...
    if (!verifyWebhookHmac(raw, hmac)) {
      console.warn("[shopify webhook customers/redact] invalid hmac", { shopDomain });
      return new NextResponse("Invalid HMAC", { status: 401 });
    }

    // Per ora non memorizziamo dati personali cliente Shopify.
    // Se in futuro li memorizziamo, qui va implementata la cancellazione.
    return new NextResponse("OK", { status: 200 });
  } catch (e) {
    console.error("[shopify webhook customers/redact] error:", e);
