// API endpoint per ricezione fatture dal Sistema di Interscambio (SDI)
// Endpoint: POST /api/sdi/ricezione-fatture
// 
// Questo endpoint riceve fatture elettroniche inviate dal SDI
// quando altri soggetti emettono fatture verso la tua partita IVA
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[SDI] Ricezione fattura ricevuta:', {
      length: xml.length,
    ...
    // Se non esiste, crea nuova fattura ricevuta
    if (!invoiceId) {
      const { data: newInvoice, error: insertError } = await supabase
        .from('invoices')
        .insert({
          type: 'received', // Fattura ricevuta (non emessa)
          number: fatturaData.numero || `SDI-${Date.now()}`,
      ...
      if (insertError) {
        console.error('[SDI] Errore inserimento fattura:', insertError);
        return createSDIResponse(
    ...
    console.log('[SDI] Fattura ricevuta salvata:', { invoice_id: invoiceId });
    ...
    console.error('[SDI] Errore ricezione fattura:', error);
    return createSDIResponse(
      {
        success: false,
