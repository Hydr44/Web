...
export async function POST(
  ...
  try {
    ...
    for (const movimento of movimenti) {
      // Debug: log movimento per troubleshooting
      console.log(`[RENTRI-MOVIMENTI] Validazione movimento ${movimento.id}:`, {
        anno: movimento.anno,
        progressivo: movimento.progressivo,
        causale_operazione: movimento.causale_operazione,
        codice_eer: movimento.codice_eer,
        data_ora_registrazione: movimento.data_ora_registrazione,
      ...
      if (!validation.valid) {
        console.error(`[RENTRI-MOVIMENTI] Errore validazione movimento ${movimento.id}:`, validation.errors);
        erroriValidazione.push({
          movimento_id: movimento.id,
          movimento_info: {
            codice_eer: movimento.codice_eer,
            causale: movimento.causale_operazione,
            progressivo: movimento.progressivo
          },
